...
#'
#' Cette fonction permet d'extraire les consultations de médecin omnipracticien
#' ou spécialiste à l'hôpital en MCO (codes prestations ACT_COD parmi : C et CS.). Les consultations dont les dates
...
#' "ben_nir_psa" (en minuscules). Les "ben_nir_psa" doivent être tous les
...
#' @param output_table_name character Le nom de la table de sortie dans la base de données. Si `output_table_name` n'est pas fourni, une table temporaire est créée.
...
extract_hospital_consultations <- function(
  ...
  if (is.null(output_table_name)) {
    output_table_name <- paste0("TMP_DISP_", format(Sys.time(), "%Y%m%d_%H%M%S"))
  }
  ...
  if (!is.null(patient_ids)) {
    ...
    try(DBI::dbRemoveTable(conn, patient_ids_table_name), silent = T)
